...
- Newsletter audiences — Dataset Options sidebar can Generate / Rebuild / Delete downloader audiences (list-only; no draft email). `wp prc datasets build-audience` remains available and can still create a draft transactional email.
...
| `POST` | `/prc-api/v3/datasets/build-audience` | `edit_post` on dataset | Builds/rebuilds a downloader audience via Firebase (persists list only; no draft email) |
