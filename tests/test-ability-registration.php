<?php
/**
 * Smoke tests for dataset ability registration metadata.
 *
 * Run with:
 *   php plugins/prc-datasets/tests/test-ability-registration.php
 */
...
namespace {
	...
	$cats   = new \PRC\Platform\Datasets\Ability_Categories( $loader );
	$ability = new \PRC\Platform\Datasets\Ability( $loader );

	$cats->register_categories();
	$ability->register_ability();

	assert_true(
		isset( $GLOBALS['__registered_categories']['datasets'] ),
		'registered datasets ability category'
	);
	assert_true(
		'Datasets' === ( $GLOBALS['__registered_categories']['datasets']['label'] ?? '' ),
		'datasets category label'
	);
	...
	foreach ( $expected as $name ) {
		...
		assert_true(
			'datasets' === ( $GLOBALS['__registered'][ $name ]['category'] ?? '' ),
			"{$name} uses datasets category"
		);
